...
- `.codex/hooks.json`：定义 LingXi 的 repo-local `UserPromptSubmit` hook
...
1. 初始化 `.lingxi/`
2. 生成或合并 `.codex/config.toml`
3. 生成或合并 `.codex/hooks.json`
4. 生成 `.codex/agents/lingxi-session-distill.toml`
5. 生成 `.lingxi/setup/automation.session-distill.toml`
6. 注册 session-distill automation

补充说明：

- 对有意义的仓库请求，LingXi 会通过仓库级 Codex `UserPromptSubmit` hook 自动注入最小必要的相关 memory。
