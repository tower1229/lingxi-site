...
对当前会话或指定时间范围内的对话做可沉淀内容提取并写入记忆库。无参数时提取**当前会话**（适合一轮对话结束后执行）；带参数时接受自然语言时间范围（如「提炼今天的会话」「提炼最近2天的会话」「1d」「24h」），解析不到有效时间范围则提示错误并终止。灵犀会汇总对应会话、经 taste-recognition 提取 payload、单次传入 lingxi-memory，最后呈现简报。
